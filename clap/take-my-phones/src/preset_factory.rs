...
// Param indices: 0=Cutoff, 1=XFeed, 2=Mix, 3=CalibrationMode
// bs2b canonical presets from https://bs2b.sourceforge.net/
struct PresetDef {
    name: &'static str,
    load_key: &'static str,
    values: [f64; PARAMS_COUNT],
}

const PRESETS: &[PresetDef] = &[
    PresetDef { name: "Default",   load_key: "default",   values: [700.0, 4.5, 1.0, 0.0] },
    PresetDef { name: "Chu Moy",   load_key: "chu-moy",   values: [700.0, 6.0, 1.0, 0.0] },
    PresetDef { name: "Jan Meier", load_key: "jan-meier", values: [650.0, 9.5, 1.0, 0.0] },
];
...
static PROVIDER_DESCRIPTOR: clap_preset_discovery_provider_descriptor_t = clap_preset_discovery_provider_descriptor {
    ...
    id: c"com.enguidanosweb.TakeMyPhones.presets".as_ptr(),
...
unsafe extern "C" fn provider_get_metadata(
    ...
    let plugin_id = clap_universal_plugin_id {
        abi: c"clap".as_ptr(),
        id: c"com.enguidanosweb.TakeMyPhones".as_ptr(),
    };
    ...
    for preset in PRESETS {
        ...
        if let Some(set_flags) = receiver_ref.set_flags {
            unsafe { set_flags(metadata_receiver, clap_preset_discovery_flags_CLAP_PRESET_DISCOVERY_IS_FACTORY_CONTENT) };
        }
    }
