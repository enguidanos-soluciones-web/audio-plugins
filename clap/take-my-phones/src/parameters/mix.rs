...
impl Parameter<Mix, Range> {
    ...
    pub fn as_draggable(&self) -> Option<ParameterDraggable<'_, Mix, Range>> {
        if self.gestures & PARAMETER_GESTURE_DRAG != 0 {
            Some(ParameterDraggable::<Mix, Range>::new(self))
        } else {
            None
        }
    }

    pub fn as_clickable(&self) -> Option<ParameterClickable<'_, Mix, Range>> {
        if self.gestures & PARAMETER_GESTURE_DOUBLE_CLICK != 0 {
            Some(ParameterClickable::<Mix, Range>::new(self))
        } else {
            None
        }
    }
