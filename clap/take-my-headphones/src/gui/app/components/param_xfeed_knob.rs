...
use crate::{
    ...
    parameters::{Parameter, Range, xfeed::XFeed},
    state::GuiRequest,
};
...
pub struct ParamXFeedWidget {
    normalized: f64,
}

impl ParamXFeedWidget {
    fn new() -> Self {
        Self { normalized: 0.0 }
    }
}

impl Widget for ParamXFeedWidget {
...
pub fn ParamXFeedKnob() -> Element {
    ...
    let normalized = Parameter::<XFeed, Range>::new().normalize(state.read().params[Parameter::<XFeed, Range>::ID]);

    let custom_widget = use_memo(|| CustomWidgetAttr::new(ParamXFeedWidget::new()));
    ...
    rsx! {
        div {
            ...
            ondoubleclick: move |_| dispatcher_cloned(GuiRequest::ResetParam(Parameter::<XFeed, Range>::ID)),
            object {
