...
use crate::parameters::center::Center;
use crate::{
...
pub struct ParamCenterWidget {
    normalized: f64,
}

impl ParamCenterWidget {
    fn new() -> Self {
        Self { normalized: 0.0 }
    }
}

impl Widget for ParamCenterWidget {
...
#[component]
pub fn ParamCenterKnob() -> Element {
    ...
    let normalized = Parameter::<Center, Range>::new().normalize(state.read().params[Parameter::<Center, Range>::ID]);

    let custom_widget = use_memo(|| CustomWidgetAttr::new(ParamCenterWidget::new()));
    ...
    rsx! {
        div {
            ...
            ondoubleclick: move |_| dispatcher_cloned(GuiRequest::ResetParam(Parameter::<Center, Range>::ID)),
            object {
