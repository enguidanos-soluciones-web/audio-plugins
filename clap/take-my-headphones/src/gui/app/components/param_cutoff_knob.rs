...
use crate::{
    ...
    parameters::{Parameter, Range, cutoff::Cutoff},
    state::GuiRequest,
};
...
pub struct ParamCutoffWidget {
    normalized: f64,
}

impl ParamCutoffWidget {
    fn new() -> Self {
        Self { normalized: 0.0 }
    }
}
...
#[component]
pub fn ParamCutoffKnob() -> Element {
    ...
    let normalized = Parameter::<Cutoff, Range>::new().normalize(state.read().params[Parameter::<Cutoff, Range>::ID]);

    let custom_widget = use_memo(|| CustomWidgetAttr::new(ParamCutoffWidget::new()));
    ...
    rsx! {
        div {
            ...
            ondoubleclick: move |_| dispatcher_cloned(GuiRequest::ResetParam(Parameter::<Cutoff, Range>::ID)),
            object {
