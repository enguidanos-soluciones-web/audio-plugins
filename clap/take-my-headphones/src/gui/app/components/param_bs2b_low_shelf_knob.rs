...
use crate::{
    ...
    parameters::{Parameter, Range, bs2b_low_shelf::Bs2bLowShelf},
    state::GuiRequest,
};
...
pub struct ParamBs2bLowShelfWidget {
    normalized: f64,
}

impl ParamBs2bLowShelfWidget {
    fn new() -> Self {
        Self { normalized: 0.0 }
    }
}

impl Widget for ParamBs2bLowShelfWidget {
...
#[component]
pub fn ParamBs2bLowShelfKnob() -> Element {
    ...
    let normalized = Parameter::<Bs2bLowShelf, Range>::new().normalize(state.read().params[Parameter::<Bs2bLowShelf, Range>::ID]);

    let custom_widget = use_memo(|| CustomWidgetAttr::new(ParamBs2bLowShelfWidget::new()));
    ...
    rsx! {
        div {
            class: "w-14 h-14",
            onmousedown: move |e| {
                dispatcher(GuiRequest::BeginGesture(Parameter::<Bs2bLowShelf, Range>::ID));
                let coords = e.data().client_coordinates();
                let raw = state.read().params[Parameter::<Bs2bLowShelf, Range>::ID];
                drag.set(ActiveDrag::from_index(Parameter::<Bs2bLowShelf, Range>::ID, coords.x, coords.y, raw));
            },
            ondoubleclick: move |_| dispatcher_cloned(GuiRequest::ResetParam(Parameter::<Bs2bLowShelf, Range>::ID)),
            object {
