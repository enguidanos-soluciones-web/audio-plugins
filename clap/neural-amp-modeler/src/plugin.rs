...
pub unsafe extern "C" fn activate(plugin: *const clap_plugin, sample_rate: f64, _min_frames_count: u32, max_frames_count: u32) -> bool {
    ...
    if let Some(path) = main_thread.selected_model_path.clone() {
        if let Ok(json) = std::fs::read_to_string(&path) {
            ...
            let loudness_correction = if dsp::nam::ffi::has_loudness(&nam_model) {
                DecibelConversion::Amplitude.to_linear(dsp::nam::TARGET_LOUDNESS_DBFS - dsp::nam::ffi::get_loudness(&nam_model))
            } else {
                1.0
            };
    ...
    plugin_ref.audio_thread = Some(AudioThreadState {
        ...
        dc_filter: DcFilter::new(5.0, sample_rate),
        ...
        match request {
            ...
            GuiRequest::OpenFileBrowser => {
                ...
                let loudness_correction = if dsp::nam::ffi::has_loudness(&model) {
                    ...
                    DecibelConversion::Amplitude.to_linear(dsp::nam::TARGET_LOUDNESS_DBFS - model_loudness)
                } else {
                    1.0
                };
            ...
            GuiRequest::ResetParam(id) => {
                ...
                snapshot_dirty = true;
                let _ = main.param_changes.push(ParamChange::Value {
                    id: change.index,
                    value: change.value,
                });
